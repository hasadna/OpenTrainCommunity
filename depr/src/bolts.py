#! /usr/bin/env python
"""Bolts for processing train data
...
class SQLAlchemyToLineSpout(Bolt):
  """
  ...
  def _datetime_to_str(self, date_val, datetime_val):
    delta = datetime_val - datetime.datetime.combine(date_val, datetime.datetime.min.time())
    total_minutes = int(delta.total_seconds() / 60)
    result = "{}{}".format(total_minutes/60, total_minutes%60)
    if result == "00":
      result = "0"
    return result
  
  def trainstop_to_line(self, trainstop):
    x = trainstop
    line = "{}\t\"{}\"\t{}\t{}\t{}\t{}\t{}\t\"{}\"".format(
        x.date.strftime('%Y%m%d'), x.train_num, 
        self._datetime_to_str(x.date, x.arrive_expected), self._datetime_to_str(x.date, x.arrive_actual),
        self._datetime_to_str(x.date, x.depart_expected), self._datetime_to_str(x.date, x.depart_actual),
        x.station_id, "stub text")
    #return "20130101  \"10\"  925  927  925  927  7300  \"aaa\""
    return line
