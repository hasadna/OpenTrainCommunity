...
class Command(BaseCommand):
    def handle(self, *args, **options):
        for day in range(7):
            self.compute_samples_per_day(day)

        for hour in range(24):
            self.compute_samples_per_hour(hour)

        for day in range(7):
            self.compute_trips_per_day(day)
    ...
    def compute_trips_per_day(self, day):
        ...
        print('Trips for day = {} % = {}'.format(day, 100.0*late_qs.count()/ok_qs.count()))
    ...
    def compute_trips_per_hour(self, hour):
        ...
        late_qs = ok_qs.filter(c__gte=300)
        print('Trips for hour = {} % = {}'.format(hour, 100.0*late_qs.count()/ok_qs.count()))
    ...
    def compute_samples_per_day(self, day):
        ...
        print("Samples for day %d Ontime (less than 5 minutes delay): %.2f " % (day+1, ontime_average))
