...
def fix_2014_file(full_filename, fixed_full_filename):
    cmd = "cat " + full_filename + " | cut -c 2-39 | " + \
        "sed -r -e 's/^.{8}/& /' |" + \
        "awk -v OFS=\"\t\" '$1=$1' > " + fixed_full_filename
    os.system(cmd)
...
if True:
    ...
    for i, filename in enumerate(files):
        print("processing file %d of %d in 2013 data" % (i + 1, len(files)))
...
if True:
    ...
    print("processing file 1 of 1 in 2014 data")
    src.manage.add_opentrain_data(fixed_full_filename)

shutil.rmtree(temp_data_dir)

elapsed = (time.time() - start)
print("setup_db done. Elapsed time:",
      str(datetime.timedelta(seconds=round(elapsed))))
